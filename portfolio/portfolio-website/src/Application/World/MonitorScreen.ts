...
export default class MonitorScreen extends EventEmitter {
    ...
    createIframe() {
        ...
        // ─────────────────────────────────────────────────────────────────
        // CUSTOMIZE: URL of the 2D "OS" inner-site that renders on the screen.
        //
        // This iframe embeds the portfolio-inner-site app. By default it now
        // points at your local dev server (localhost:3000) so it works out of
        // the box while developing. Once you deploy the inner-site (e.g. to
        // Vercel/Netlify), set OS_PROD_URL to your hosted URL — that becomes
        ...
        // local dev server so the screen still renders during development.
        iframe.src = OS_PROD_URL || OS_DEV_URL;

        /**
         * Force the dev server when the ?dev query param is present.
         *
         * Warning: This will not work unless the dev server is running on
         * localhost:3000. Running the dev server also causes browsers to flag
         * mixed-content / insecure-connection warnings inside the iframe.
         */
        const urlParams = new URLSearchParams(window.location.search);
        if (urlParams.has('dev')) {
            iframe.src = OS_DEV_URL;
        }
